...
void main() {
    ...
    vec3 earthLights = texture(texture_lights, TexCoord).rgb;

    vec3 normal = (2.f * texture(texture_normal, TexCoord).rgb) - vec3(1.f);
    ...
    vec3 result = ((ambientColor*objectColor) + (((1.f-shadow)+0.25)*max(Fs,0.f)));
    //vec3 result = (ambientColor + (Fs));

    float lum = luminance(max(Fs, 0.f)).x*20.f;
    result += earthLights * exp(-lum) * vec3(0.99609375, 0.87109375, 0.38671875) * 2.f;
